...
async fn main() -> anyhow::Result<()>{
    ...
    let server_2 = async {
      ...
      let _re = join(start_test_server("hello 2", tokio::time::sleep(Duration::from_secs(100))),
      async {
          tokio::time::sleep(Duration::from_secs(10)).await;
...
async fn start_test_server(resp:&'static str, signal: impl Future<Output = ()> + Send + 'static) -> anyhow::Result<()>{
    ...
    server.serve(make_svc).with_graceful_shutdown(signal).await?;
    Ok(())
}
...
async fn simple_test() -> anyhow::Result<()> {
    ...
    let (tx,rx) = oneshot::channel::<()>();
    let (_adder, server) = warp::serve(routes).bind_with_graceful_shutdown(([0,0,0,0], 3030), async {
        rx.await.ok();
    });
