...
use hyper::{Body, header, Method, Response, StatusCode};
...
lazy_static! {
    static ref ALLOW_METHODS: HashSet<Method> = HashSet::from([Method::GET, Method::OPTIONS, Method::HEAD]);
}
...
pub fn cors_resp(mut res: Response<Body>, origin:HeaderValue) -> Response<Body>{
    ...
    let access_control_allow_methods: AccessControlAllowMethods = ALLOW_METHODS.iter().cloned().collect();
...
pub fn resp_cors_request(method: &Method, headers: &HeaderMap, allow_cors: bool) -> Either<Validated, Response<Body>> {
    match (headers.get(header::ORIGIN), method) {
        (Some(origin), &http::Method::OPTIONS) => {
            ...
            if let Some(req_method) = headers.get(header::ACCESS_CONTROL_REQUEST_METHOD) {
                if !http::Method::from_bytes(req_method.as_bytes()).map(|method| ALLOW_METHODS.contains(&method)).unwrap_or(false) {
